...
// Also import Camera for avatar upload
if (!content.includes('Camera')) {
  content = content.replace(
    "Key, MessageSquare } from 'lucide-react';",
    "Key, MessageSquare, Camera } from 'lucide-react';"
  );
}

// Add MockDB import
if (!content.includes('import { MockDB }')) {
  content = content.replace(
                  ...
                  onChange={(e) => {
                    ...
                    reader.onload = (ev) => {
                      ...
                      if (dataUrl && user) {
                        ...
                        // Hack to force re-render, reload page to sync everything
