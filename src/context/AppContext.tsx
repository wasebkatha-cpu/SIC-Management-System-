...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [enforcementActions, setEnforcementActions] = useState<EnforcementActionRecord[]>(initialEnforcementActions);
  const [readers, setReaders] = useState<ReaderData[]>(initialReaders);
  const [publicBodies, setPublicBodies] = useState<PublicBody[]>(initialMockPublicBodies);
  ...
  const updatePublicBody = (id: string, updates: Partial<PublicBody>) => {
    setPublicBodies(prev => prev.map(pb => pb.id === id ? { ...pb, ...updates } : pb));
  ...
  const addEnforcementAction = (action: Omit<EnforcementActionRecord, 'id'>) => {
    const newAction: EnforcementActionRecord = {
      ...action,
      id: `ea-${Date.now()}`
    };
  ...
  const addProceeding = (
    ...
    setComplaints(prev => prev.map(c => {
      if (c.complaintNo === complaintNo) {
        const nextSrNo = c.proceedings.length > 0
