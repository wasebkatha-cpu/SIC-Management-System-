  ...
  const renderLogTable = (logsToRender: MockLog[], emptyMessage: string) => (
    <div className="bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
      <table className="w-full text-left text-sm">
        <thead className="bg-neutral-50 dark:bg-slate-800/90 text-neutral-600 dark:text-neutral-300 font-medium border-b border-neutral-200 dark:border-slate-800">
          <tr>
        ...
        <tbody className="divide-y divide-neutral-100 dark:divide-slate-800/80">
          {loading ? (
            <tr><td colSpan={4} className="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">Loading activity...</td></tr>
          ) : logsToRender.length === 0 ? (
            <tr><td colSpan={4} className="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">{emptyMessage}</td></tr>
          ) : (
            logsToRender.map(log => (
              <tr key={log.id} className="hover:bg-neutral-50 dark:hover:bg-slate-800/50">
                <td className="px-6 py-4 text-neutral-500 dark:text-neutral-400 font-mono text-xs">{formatTime(log.timestamp)}</td>
                ...
                <td className="px-6 py-4 text-neutral-700 dark:text-neutral-300">{log.activity}</td>
              </tr>
            ))
          )}
        </tbody>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1200px] mx-auto space-y-6">
        ...
        {(user?.role === 'superUser' || user?.role === 'admin') && (
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="border border-neutral-300 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-600 focus:border-transparent bg-white dark:bg-slate-900 text-neutral-900 dark:text-white shadow-sm"
            >
      ...
      <div className="space-y-8">
        <div>
          <h2 className="text-lg font-semibold text-neutral-900 dark:text-white mb-4">Today's Activity</h2>
          {renderLogTable(todaysLogs, "No activity recorded today.")}
        </div>

        <div>
          <h2 className="text-lg font-semibold text-neutral-900 dark:text-white mb-4">Previous Activity</h2>
