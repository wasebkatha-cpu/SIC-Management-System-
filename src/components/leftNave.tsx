...
import { ChevronLeft, ChevronRight, LayoutDashboard, AlertCircle, Building2, UserCheck, ClipboardList, Users, Shield, LogOut, History, Key, MessageSquare, Camera, Edit2, Sun, Moon, BookOpen, BellRing, Archive } from 'lucide-react';
    ...
    >
      ...
      <div className={`flex-1 py-4 px-2.5 flex flex-col justify-start ${isExpanded ? 'overflow-y-auto overflow-x-hidden' : 'overflow-visible'}`}>
        <nav className={`space-y-1 ${isExpanded ? 'overflow-hidden' : 'overflow-visible'}`}>
          ...
          {hasPermission('inwardOutward') && (
            <NavItem
              label="Inward / Outward"
              icon={Archive}
              active={activeView === 'inwardOutward'}
              activeClass="dark:bg-indigo-950/70 text-neutral-900 dark:text-white dark:border-indigo-800/60"
              onClick={() => onSelect && onSelect('inwardOutward')}
              isExpanded={isExpanded}
            />
          )}
