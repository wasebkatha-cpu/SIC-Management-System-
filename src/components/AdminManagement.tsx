...
import { Shield, Plus, Trash2, X, User as UserIcon, Edit2, UserCheck } from 'lucide-react';
          ...
          {isAdding && (
            <div className="bg-white dark:bg-gradient-to-b dark:from-slate-900/95 dark:to-indigo-950/40 border border-neutral-200 dark:border-indigo-900/50 rounded-xl shadow-sm p-6">
              ...
              <form onSubmit={handleAddAdmin} className="max-w-md space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-neutral-700 dark:text-white mb-2">Role Based Access (Permissions)</label>
                  <div className="bg-neutral-50 dark:bg-slate-900/80 p-4 rounded-md border border-neutral-200 dark:border-slate-700 max-h-[300px] overflow-y-auto">
                    <div className="space-y-6">
                      {PERMISSION_GROUPS.map(group => (
                        <div key={group.module}>
                          <h4 className="text-xs font-semibold text-neutral-500 dark:text-neutral-300 uppercase tracking-wider mb-2 border-b border-neutral-200 dark:border-slate-800 pb-1">{group.module}</h4>
                          <div className="space-y-1.5 pl-1">
                            {group.permissions.map(perm => (
                              <label key={perm.id} className="flex items-center gap-3 cursor-pointer p-1 hover:bg-neutral-100 dark:hover:bg-slate-800 rounded">
                                <input
                                  type="checkbox"
                                  checked={selectedPermissions.includes(perm.id)}
                                  onChange={() => handleTogglePermission(perm.id)}
                                  className="w-4 h-4 text-blue-600 rounded border-neutral-300 dark:border-slate-600 focus:ring-blue-500"
                                />
                                <span className="text-sm font-medium text-neutral-700 dark:text-white">{perm.label}</span>
                              </label>
                            ))}
                          </div>
                        </div>
                  ...
                  <p className="text-xs text-neutral-500 dark:text-neutral-300 mt-1">Select the sections and granular actions this admin is allowed to access.</p>
                </div>
