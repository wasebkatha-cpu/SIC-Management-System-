...
export default function Complaints({ onSelectComplaint }: { onSelectComplaint?: (id: string) => void }) {
  ...
  if (activeForm) {
    const editData = activeForm !== 'new' ? complaints.find(c => c.complaintNo === activeForm) : undefined;
    return <ComplaintForm onBack={() => setActiveForm(null)} initialData={editData} />;
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
          ...
          {(!user || user.role === 'superUser' || hasPermission('complaints_create')) && (
            <button
      ...
      <div className="bg-white dark:bg-gradient-to-b dark:from-slate-900/95 dark:to-blue-950/30 border border-neutral-200 dark:border-blue-900/50 rounded-lg shadow-sm overflow-hidden flex flex-col min-h-[480px]">
        <div className="flex-1 overflow-auto">
          <table className="w-full text-left text-xs text-neutral-600 dark:text-neutral-200">
            <thead className="bg-neutral-50 dark:bg-slate-800/90 border-b border-neutral-200 dark:border-blue-900/40 text-neutral-900 dark:text-white sticky top-0 z-10">
              <tr>
                <th className="px-2 py-2 font-medium text-center w-10">Sr. No.</th>
                <th className="px-2 py-2 font-medium">Complaint No.</th>
                ...
                <th className="px-2 py-2 font-medium">Previous Hearing</th>
                <th className="px-2 py-2 font-medium">Next Hearing</th>
                <th className="px-2 py-2 font-medium" title="Most recent notice/order or whatever issued">
                  <div className="flex items-center gap-1">
                    <FileText className="w-3 h-3 text-teal-500" />
                    <span>Current Stage (Last Order)</span>
                  </div>
                </th>
                <th className="px-2 py-2 font-medium max-w-[150px]">Remarks</th>
                <th className="px-2 py-2 font-medium w-[60px] text-center">Edit</th>
              </tr>
            </thead>
              ...
              {filteredComplaints.length > 0 ? (
                filteredComplaints.map((complaint, index) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-2 py-2 text-center dark:text-neutral-300 font-mono text-[11px]">{index + 1}</td>
                      <td className="px-2 py-2 font-medium text-neutral-900 dark:text-white whitespace-nowrap group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                        {complaint.complaintNo}
                      </td>
                      <td className="px-2 py-2 font-medium text-neutral-800 dark:text-white">
                        {complaint.complainantName}
                      </td>
                      <td className="px-2 py-2">
                        <div className="font-medium text-neutral-800 dark:text-white line-clamp-2 leading-tight">{complaint.respondentName}</div>
                        {complaint.designatedOfficialName && (
                          <div className="text-[10px] text-neutral-500 dark:text-neutral-300 mt-0.5">DO: {complaint.designatedOfficialName}</div>
                        )}
                      </td>
                      <td className="px-2 py-2">
                        {resolvedDistrict || resolvedDivision ? (
                          <div>
                            ...
                            {resolvedDivision && (
                              <div className="text-[10px] text-neutral-500 dark:text-neutral-300 mt-0.5">
                      ...
                      <td className="px-2 py-2 whitespace-nowrap text-xs text-neutral-700 dark:text-neutral-200 font-medium">
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] bg-purple-50 text-purple-700 dark:bg-purple-950/60 dark:text-purple-300 border border-purple-200 dark:border-purple-800/60">
                          {readerName}
                        </span>
                      </td>
                      <td className="px-2 py-2 whitespace-nowrap dark:text-neutral-300">{complaint.previousHearingDate || '-'}</td>
                      <td className="px-2 py-2 whitespace-nowrap font-medium text-emerald-700 dark:text-emerald-400">{complaint.nextHearingDate || '-'}</td>
                      <td className="px-2 py-2 whitespace-nowrap">
                        <span 
                          className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-700/20 dark:bg-blue-950/80 dark:text-blue-300 dark:ring-blue-700/50" 
                          title={`Last Order / Current Stage: ${currentStage}`}
                        >
                          {currentStage}
                        </span>
                      </td>
                      <td className="px-2 py-2 text-[10px] dark:text-neutral-300 max-w-[150px] truncate" title={complaint.remarks}>
                        {complaint.remarks}
                      </td>
                      <td className="px-2 py-2 text-center">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setActiveForm(complaint.complaintNo);
                          }}
                          className="p-1.5 text-neutral-400 hover:text-blue-600 dark:text-slate-500 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/40 rounded transition-colors"
