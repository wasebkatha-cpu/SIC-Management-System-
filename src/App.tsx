...
import InwardOutwardRegistry from './components/inwardOutwardRegistry';
...
export default function App() {
    ...
    const checkRole = async () => {
      try {
        if (window.electronAPI) {
          const role = await window.electronAPI.getAppRole();
          setAppRole(role);
        } else {
          setAppRole(localStorage.getItem('app_role'));
        }
      } catch (err) {
        console.error('Failed to get app role', err);
      } finally {
  ...
  // If role is not set, force Setup Wizard
  if (!appRole) {
    return <SetupWizard onComplete={(role) => setAppRole(role)} />;
  }
      ...
      <main className="flex-1 h-full overflow-y-auto overflow-x-auto min-w-0 bg-neutral-50 dark:bg-[#090d16]">
        {!hasPermission(activeView) && activeView !== 'adminManagement' ? (
          ...
          <>
            ...
            {activeView === 'inwardOutward' && hasPermission('inwardOutward') && <InwardOutwardRegistry />}
            {activeView === 'readers' && hasPermission('readers') && <Reader />}
            {activeView === 'adminManagement' && (user.role === 'superUser' || hasPermission('adminManagement')) && <AdminManagement />}
