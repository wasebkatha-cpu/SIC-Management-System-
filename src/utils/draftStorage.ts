...
export function getStoredDrafts(): NoticeDraftRecord[] {
  const drafts = getStoreData('drafts');
  return drafts || [];
}
...
export function saveNoticeDraft(draft: NoticeDraftRecord): NoticeDraftRecord {
  ...
  setStoreData('drafts', updatedList);
  ...
  // Sync to PostgreSQL Express API
  apiFetch('/api/drafts', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(draft)
  }).catch(e => console.error('Failed to sync draft to API:', e));
...
export function markDraftAsRead(draftId: string): void {
  ...
  if (changed) {
    setStoreData('drafts', updated);
    window.dispatchEvent(new CustomEvent('notice_drafts_updated', { detail: updated }));
    
    // Sync updated draft to PostgreSQL API
    const updatedDraft = updated.find(d => d.id === draftId);
    if (updatedDraft) {
      apiFetch('/api/drafts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updatedDraft)
      }).catch(e => console.error('Failed to sync draft read status to API:', e));
    }
  }
}
...
export function recordDraftDispatch(
  ...
  apiFetch('/api/drafts', {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(updatedDraft)
  }).catch(e => console.error('Failed to sync draft dispatch to API:', e));
...
export function deleteDraft(draftId: string): void {
  ...
  setStoreData('drafts', filtered);
  window.dispatchEvent(new CustomEvent('notice_drafts_updated', { detail: filtered }));

  apiFetch(`/api/drafts/${draftId}`, {
    method: 'DELETE'
  }).catch(e => console.error('Failed to delete draft from API:', e));
}

// Initial fetch to sync down drafts
export function initDraftsSync(): void {
  apiFetch('/api/drafts')
    .then(res => res.json())
    .then(data => {
      setStoreData('drafts', data);
      window.dispatchEvent(new CustomEvent('notice_drafts_updated', { detail: data }));
    })
    .catch(e => console.error('Failed to init drafts from API:', e));
}
